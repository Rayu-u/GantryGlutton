namespace GantryGlutton {
  ...
  export class Gantry extends f.ComponentScript {
    ...
    public start = (_event: Event): void => {
      let platformTransform: f.ComponentRigidbody = this.node.getChildrenByName("Platform")[0].getComponent(f.ComponentRigidbody);
      
      this.node.getChildrenByName("Bridge")[0].getComponent(GantryBridge).platformRigidbody = platformTransform;
    };
  }
}
